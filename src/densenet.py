# Copyright 2019 BM AI. All Rights Reserved.
#
...
def _conv_block(inputs, num_filters, data_format='NHWC', scope=None, outputs_collections=None):
    with tf.variable_scope(scope, 'conv_blockx', [inputs]) as sc:
        net = inputs
        # _conv(inputs, num_filters, kernel_size, stride=1, dropout_rate=None,scope=None, outputs_collections=None):
...
@slim.add_arg_scope
#  growth_rate表示每个dense block中每层输出的feature map个数,就是卷积核的个数
...
def densenet(inputs,
    ...
    with tf.variable_scope(scope, 'densenetxxx', [inputs, num_classes], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.batch_norm, slim.dropout],
                            is_training=is_training), slim.arg_scope(
            ...
            # initial convolution  7x7 conv、2 stride -> 3x3 max_pool、2 stride
            net = slim.conv2d(net, num_filters, 7, stride=2, scope='conv1')  # num_filters个卷积核,卷积核大小为7*7
            net = slim.batch_norm(net)
            net = tf.nn.relu(net)
            net = slim.max_pool2d(net, 3, stride=2, padding='SAME')

            # blocks  exp: [6, 12, 24, 16]
...
def densenet_for_fine_tuning(
    ...
    with tf.variable_scope(scope, 'densenetxxx', [inputs, num_classes], reuse=reuse) as sc:
        end_points_collection = sc.name + '_end_points'
        with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training), slim.arg_scope(
            [slim.conv2d, _conv, _conv_block, _dense_block, _transition_block],
            outputs_collections=end_points_collection), slim.arg_scope([_conv], dropout_rate=dropout_rate):
            net = inputs
            ...
            net = tf.nn.relu(net)
            net = slim.max_pool2d(net, 3, stride=2, padding='SAME')
            # print("initial convolution:", net.shape)

            # blocks  exp: [6, 12, 24, 16]
            for i in range(num_dense_blocks - 1):  # [6, 12, 24]
                # dense blocks
                # growth_rate表示每个dense block中每层输出的feature map个数,就是卷积核的个数
                # num_filters表示拼接之后的feature map个数
                net, num_filters = _dense_block(net, num_layers[i], num_filters,
                                                growth_rate,
                                                scope='dense_block' + str(i + 1))
                ...
                net, num_filters = _transition_block(net, num_filters,
                                                     compression=compression,
                                                     scope='transition_block' + str(i + 1))
                # print('dense_block' + str(i + 1), net.shape)
            # 16
            net, num_filters = _dense_block(
                net, num_layers[-1], num_filters,
                growth_rate,
                scope='dense_block' + str(num_dense_blocks))
            # print('dense_block' + str(num_dense_blocks), net.shape)
...
def densenet121_fine_tuning(inputs, num_classes=1000, data_format='NHWC', is_training=True, reuse=None):
    return densenet_for_fine_tuning(inputs,
                                    reduction=0.5,
                                    growth_rate=32,
                                    num_filters=64,
                                    num_layers=[6, 12, 24, 16],
...
def densenet161_fine_tuning(inputs, num_classes=1000, data_format='NHWC', is_training=True, reuse=None):
    return densenet_for_fine_tuning(inputs,
                                    reduction=0.5,
                                    growth_rate=48,
                                    num_filters=96,
...
def densenet169_fine_tuning(inputs, num_classes=1000, data_format='NHWC', is_training=True, reuse=None):
    net = densenet_for_fine_tuning(inputs,
                                   reduction=0.5,
                                   growth_rate=32,
                                   num_filters=64,
                                   num_layers=[6, 12, 32, 32],
                                   ...
                                   reuse=reuse,
                                   scope='densenet169')
    print("densenet169_fine_tuning:", net.shape)
    return net
