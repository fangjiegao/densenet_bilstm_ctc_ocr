...
tf.app.flags.DEFINE_float('learning_rate', 5e-1, '')
...
tf.app.flags.DEFINE_string(
    # 'image_path', '/Users/sherry/data/Synthetic_Chinese_String_Dataset_part', 'data path')
    'image_path', '/home/gaofangjie/Synthetic_Chinese_String_Dataset', 'data path')
tf.app.flags.DEFINE_string(
    # 'label_path', '/Users/sherry/data/Synthetic_Chinese_String_Dataset_label', 'label path')
...
def main(argv=None):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(FLAGS.training_epochs):
            print("epoch:", epoch)
            for step in range(restore_step, 1010):
                ...
                if step % 1000 == 0:  # 1000张图片或每迭代10轮保存一次模型
                    filename = ('ocr_' + '{:d}'.format(epoch) + '_' + '{:d}'.format(step) + '.ckpt')
